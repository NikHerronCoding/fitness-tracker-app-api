...
- [x] Add a protected `GET /workouts` route in `routes/api.php` under the existing `auth:sanctum` group.
- [x] Create `WorkoutSessionController@index` to fetch the authenticated user's sessions using `WorkoutSession::forUser(...)` and eager-load entries, sets, and exercise details.
